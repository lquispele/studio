...
2.  **ABSOLUTE TOP PRIORITY: STRICTLY AVOID BLOCKED AREAS.** Your single most important objective is to generate a conceptual path that *completely and unequivocally* avoids all areas covered by the 'blockedRouteInfo'. Use their names and street descriptions to understand these no-go zones. **It is more important to avoid these blocked areas than to find the shortest or most direct path, even if the resulting detour is significantly longer.** The waypoints you generate for the 'coordinates' field MUST define a path that navigates entirely AROUND these blocked areas, not through any segment of them. If a direct line between your origin and destination (or between any two consecutive waypoints you generate) would intersect a blocked area, you MUST insert additional waypoints to create a clear and unambiguous detour. Only *after* guaranteeing complete avoidance of all blocked routes should you consider minimizing travel through highly congested admin-defined routes.
...
5.  Explain your reasoning for choosing this path. **Crucially, you MUST detail how your suggested path and waypoints successfully and strictly navigate around ALL described blocked areas, even if this results in a significantly longer or less direct route.** Also reference street names or areas if relevant to how you considered congestion (which is a secondary concern to avoiding blockages).
